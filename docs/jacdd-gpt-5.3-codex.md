[LAPC v1 | source: docs/jacdd-reference.md + docs/raw-braindump-extraction.md | ratio: 1303/321 (wc-w proxy; ~4.06:1)]
...
loop: estimate→interview(3-5 Qs, one boundary at a time)→constrain(2-4 actionable constraints with max Dice impact)→re-estimate→repeat until HIGH or "good enough".
...
constraint cue: constraints should be actionable and sparse; maximize Dice gain per added constraint.
